"""
This is a script to plot the results of the impacting stick model. It requires the specification of the directory from which to load the results, as well as the method. It then loads the results and plots the velocities, 
...
for method in ['MJ', 'MJF']:
    # Get the load folder
    load_folder = data_folder + 'Rod/' + method + '/'
    # Load the data
    ...
    ax2.plot(t, u[:, 0], linestyle=line_split[0], label=r'$u_{\hbox{\tiny{N}}}$')
    ax2.plot(t, u[:, 1], linestyle=line_split[1], label=r'$u_{\hbox{\tiny{N}}}$')
    ax2.set_xlim([0, t[-1]])
    ax2.set_xlabel(r'$t$')
    ...
    ax3.plot(t, p[:, 0], linestyle=line_split[0], label=r'$p_{\hbox{\tiny{N}}}$')
    ax3.plot(t, p[:, 1], linestyle=line_split[1], label=r'$p_{\hbox{\tiny{T}}}$')
    ax3.set_xlim([0, t[-1]])
    ax3.set_xlabel(r'$t$')
    ax3.set_ylim([-0.05, 1.1*np.max(p)])
    ...
    ax4.plot(t, work_normal, linestyle=line_split[0], label=r'$w_{\hbox{\tiny{N}}}$')
    ax4.plot(t, work_tangent, linestyle=line_split[1], label=r'$w_{\hbox{\tiny{T}}}$')
    ax4.set_xlim([0, t[-1]])
    ax4.set_xlabel(r'$t$')
    if method == 'MJ':
        ...
        ax4.set_ylim([1.1*np.min(work_tangent), 0.0002])
        plt.legend(loc=4)
    ...
    ax5.set_xlim([0, t[-1]])
    ax5.set_xlabel(r'$t$')
    ax5.set_ylim([-0.05, 1.2*np.max(total_energy)])
    ax5.set_ylabel(r'Energy')
    ax5.minorticks_on()
    ax5.text(text_x_pos, text_y_pos, r'$(f)$', transform=ax5.transAxes, fontsize=10)
    plt.legend(loc=6, ncol=2)

    # Tighten the layout
    plt.tight_layout()

    # Save as a pdf
    save_file_name = figure_folder + '_'.join(['Rod', method, 'full_variables', 'version', '1']) + '.pdf'
