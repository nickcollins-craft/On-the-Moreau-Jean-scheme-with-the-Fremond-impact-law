"""
This is a code that performs the Moreau–Jean linear one-step integration scheme for a nonsmooth contact dynamics problem. The function is called from another code (main.py) which maintains the global history. The 
function takes the initial and final times, initial positions and velocities, the θ parameter, the model, and the tolerance for the solver as inputs, and returns the final velocity and position, the work done by the 
...
def MoreauJeanLinearOneStep(t_i, t_f, q_i, v_i, θ, Model, tol):
    """
    This function implements the Moreau-Jean time-stepping scheme for a nonsmooth contact dynamics problem.
    ...
    f_ext = Model.f_ext
    # Get the gradient of the "internal forces" (the stiffness matrix and the damping matrix) from the model
    ...
    # Calculate the unmodified free impulse (in the absence of contact) at the end of the time step
    R_free_unmodified = h*(θ*f_ext(t_f) + (1 - θ)*f_ext(t_i)) + (M - (h**2)*θ*(1 - θ)*K - h*(1 - θ)*C) @ v_i - h*K @ q_i
    # If the model has displacement controlled degrees of freedom, the boundary conditions need to be enforced
    v_f = np.zeros_like(v_i)
    if Model._isDisplacementControl:
        # Impose the velocity boundary conditions at the end of the time step
        v_f[Model._fixed_dofs] = Model.fixed_dof_values(t_f)
    ...
    # Calculate the free velocity (in the absence of contact) at the end of the time step
    ...
    if n_active > 0:
        ...
        W = H_active @ M_hat_inv @ H_active.T
    ...
    # Calculate the global final velocity and position
    v_f = v_free + M_hat_inv @ P_f
